...
describe("MicroagentManagement", () => {
  ...
  const mockRepositories: GitRepository[] = [
    ...
    {
      id: "2",
      full_name: "user/repo2/.thinksoft",
      git_provider: "github",
      is_public: true,
      owner_type: "user",
    ...
    {
      id: "7",
      full_name: "user/gitlab-repo/thinksoft-config",
      git_provider: "gitlab",
      is_public: true,
      owner_type: "user",
    ...
    {
      id: "8",
      full_name: "org/gitlab-org-repo/thinksoft-config",
      git_provider: "gitlab",
      is_public: true,
      owner_type: "organization",
  ...
  // Helper function to filter repositories with Thinksoft suffixes
  const getRepositoriesWithThinksoftSuffix = (
    repositories: GitRepository[],
  ) => {
    return repositories.filter(
      (repo) =>
        repo.full_name.endsWith("/.thinksoft") ||
        repo.full_name.endsWith("/thinksoft-config"),
    );
  };
  ...
  const mockMicroagents: RepositoryMicroagent[] = [
    {
      ...
      git_provider: "github",
      path: ".thinksoft/microagents/test-microagent-1",
    },
    {
      name: "test-microagent-2",
      created_at: "2021-10-02T12:00:00Z",
      git_provider: "github",
      path: ".thinksoft/microagents/test-microagent-2",
    },
  ];
  ...
  const mockConversations: Conversation[] = [
    {
      ...
      selected_repository: "user/repo2/.thinksoft",
  ...
  beforeEach(() => {
    ...
    // Mock the search repositories hook to return repositories with Thinksoft suffixes
    const mockSearchResults =
      getRepositoriesWithThinksoftSuffix(mockRepositories);
    ...
    vi.spyOn(GitService, "getRepositoryMicroagentContent").mockResolvedValue({
      ...
      path: ".thinksoft/microagents/update-test-microagent",
      git_provider: "github",
  ...
  it("should display repositories in accordion", async () => {
    ...
    expect(repo1).toHaveTextContent("user/repo2/.thinksoft");
  });
  ...
  it("should display microagent cards with correct information", async () => {
    ...
    const filePath1 = screen.getByText(
      ".thinksoft/microagents/test-microagent-1",
    );
    const filePath2 = screen.getByText(
      ".thinksoft/microagents/test-microagent-2",
    );
  ...
  it("should display ready to add microagent message in main area", async () => {
    ...
    const descriptionMessage = screen.getByText(
      "MICROAGENT_MANAGEMENT$THINKSOFT_CAN_LEARN_ABOUT_REPOSITORIES",
    );
  ...
  describe("Search functionality", () => {
    ...
    it("should filter repositories when typing in search input", async () => {
      ...
      // Initially only repositories with .thinksoft should be visible
      expect(screen.getByTestId("repository-name-tooltip")).toBeInTheDocument();
      ...
      expect(screen.queryByText("user/repo1")).not.toBeInTheDocument();
      expect(
        screen.queryByText("org/repo3/.thinksoft"),
      ).not.toBeInTheDocument();
      expect(screen.queryByText("user/repo4")).not.toBeInTheDocument();
    ...
    it("should show all repositories when search input is cleared", async () => {
      ...
      // All repositories should be visible again (only those with .thinksoft)
      expect(screen.getByTestId("repository-name-tooltip")).toBeInTheDocument();
      expect(screen.queryByText("user/repo1")).not.toBeInTheDocument();
      expect(
        screen.queryByText("org/repo3/.thinksoft"),
    ...
    it("should handle empty search results", async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByText("user/repo1")).not.toBeInTheDocument();
        expect(
          screen.queryByText("user/repo2/.thinksoft"),
        ).not.toBeInTheDocument();
        expect(
          screen.queryByText("org/repo3/.thinksoft"),
        ).not.toBeInTheDocument();
    ...
    it("should handle special characters in search", async () => {
      ...
      await user.type(searchInput, ".thinksoft");

      // Only repositories with .thinksoft should be visible
      expect(screen.getByTestId("repository-name-tooltip")).toBeInTheDocument();
      expect(screen.queryByText("user/repo1")).not.toBeInTheDocument();
      expect(screen.queryByText("user/repo4")).not.toBeInTheDocument();
    });
  ...
  describe("Microagent management conversations functionality", () => {
    ...
    it("should display microagent file paths for microagents but not for conversations", async () => {
      ...
      expect(
        screen.queryByText(".thinksoft/microagents/Test Conversation 1"),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText(".thinksoft/microagents/Test Conversation 2"),
      ).not.toBeInTheDocument();
    });
  ...
  describe("MicroagentManagementMain", () => {
    const mockRepositoryMicroagent: RepositoryMicroagent = {
      ...
      created_at: "2021-10-01T12:00:00Z",
      git_provider: "github",
      path: ".thinksoft/microagents/test-microagent",
    };
    ...
    it("should render MicroagentManagementViewMicroagent when microagent is selected", async () => {
      ...
      expect(
        screen.getByText(".thinksoft/microagents/test-microagent"),
      ).toBeInTheDocument();
    });
    ...
    it("should handle microagent with all required properties", async () => {
      const completeMicroagent: RepositoryMicroagent = {
        ...
        created_at: "2021-10-01T12:00:00Z",
        git_provider: "github",
        path: ".thinksoft/microagents/complete-microagent",
      };
      ...
      expect(
        screen.getByText(".thinksoft/microagents/complete-microagent"),
      ).toBeInTheDocument();
    });
    ...
    it("should handle conversation with all required properties", async () => {
      const completeConversation: Conversation = {
        ...
        selected_repository: "user/complete-repo/.thinksoft",
        selected_branch: "main",
        git_provider: "github",
        last_updated_at: "2021-10-01T12:00:00Z",
        created_at: "2021-10-01T12:00:00Z",
        ...
        data: [
          {
            ...
            git_provider: "github",
            path: ".thinksoft/microagents/test",
          },
        ],
        isLoading: false,
        isError: false,
  ...
  describe("Learn something new button functionality", () => {
    const mockMicroagentForLearn: RepositoryMicroagent = {
      ...
      created_at: "2021-10-01T12:00:00Z",
      git_provider: "github",
      path: ".thinksoft/microagents/learn-test-microagent",
    };
