 ...
 *
 * @example
 * constructMicroagentUrl("github", "owner/repo", ".thinksoft/microagents/tell-me-a-joke.md")
 * // "https://github.com/owner/repo/blob/main/.thinksoft/microagents/tell-me-a-joke.md"
 ...
 * constructMicroagentUrl("bitbucket", "owner/repo", ".thinksoft/microagents/docker-helper.md")
 * // "https://bitbucket.org/owner/repo/src/main/.thinksoft/microagents/docker-helper.md"
 */
...
export const getRepoMdCreatePrompt = (
  ...
  return `Please explore this repository. Create the file .thinksoft/microagents/repo.md with:
            ${
              query
                ? `- ${query}`
...
/**
 * Get the Thinksoft query string based on the provider
 * @param provider The git provider
 * @returns The query string for searching Thinksoft repositories
 */
export const getThinksoftQuery = (provider: Provider | null): string => {
  const providerRepositorySuffix: Record<string, string> = {
    gitlab: "thinksoft-config",
    azure_devops: "thinksoft-config",
    default: ".thinksoft",
  } as const;
...
/**
 * Check if a repository has the Thinksoft suffix based on the provider
 * @param repo The Git repository to check
 * @param provider The git provider
 * @returns True if the repository has the Thinksoft suffix
 */
export const hasThinksoftSuffix = (
  repo: GitRepository,
  provider: Provider | null,
): boolean => repo.full_name.endsWith(`/${getThinksoftQuery(provider)}`);
