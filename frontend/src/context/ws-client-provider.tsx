...
import { ThinksoftParsedEvent } from "#/types/core";
import {
...
const isThinksoftEvent = (event: unknown): event is ThinksoftParsedEvent =>
  typeof event === "object" &&
  event !== null &&
  "id" in event &&
  "source" in event &&
...
const isCommandAction = (event: ThinksoftParsedEvent): event is CommandAction =>
...
export function WsClientProvider({
  ...
  function handleMessage(event: Record<string, unknown>) {
    ...
    if (isThinksoftEvent(event)) {
      ...
      if (isThinksoftAction(event) || isThinksoftObservation(event)) {
        addEvent(event); // Event is already ThinksoftParsedEvent
      }
