...
const shouldInvalidateConversationsList = (currentSocketEvent: unknown) => {
  ...
  const hasStateChanged =
    isThinksoftEvent(currentSocketEvent) &&
    isAgentStateChangeObservation(currentSocketEvent);
  const hasFinished =
    isThinksoftEvent(currentSocketEvent) && isFinishAction(currentSocketEvent);
...
- Step 1: Create a markdown file inside the .thinksoft/microagents folder with the name of the microagent (The microagent must be created in the .thinksoft/microagents folder and should be able to perform the described task when triggered). This is the instructions about what the microagent should do: ${formData.query}. ${
...
- Step 1: Update the microagent. This is the path of the microagent: ${formData.microagentPath} (The updated microagent must be in the .thinksoft/microagents folder and should be able to perform the described task when triggered). This is the updated instructions about what the microagent should do: ${formData.query}. ${
  formData.triggers && formData.triggers.length > 0
    ? `This is the triggers of the microagent: ${formData.triggers.join(", ")}`
    : "Please be noted that the microagent doesn't have any triggers."
}
...
export function MicroagentManagementContent() {
  ...
  const handleMicroagentEvent = React.useCallback(
    (socketEvent: unknown) => {
      ...
      if (isThinksoftEvent(socketEvent) && isFinishAction(socketEvent)) {
