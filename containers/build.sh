...
if [[ -n $GITHUB_REF_NAME ]]; then
  ...
  THINKSOFT_BUILD_VERSION=$sanitized_ref_name
...
if [[ $dry_run -eq 1 ]]; then
  ...
  jq -n \
    ...
    --arg thinksoft_build_version "$THINKSOFT_BUILD_VERSION" \
    --arg dockerfile "$dir/Dockerfile" \
    '{
      tags: $tags,
      platform: $platform,
      build_args: [
        "THINKSOFT_BUILD_VERSION=" + $thinksoft_build_version
      ],
...
docker buildx build \
  $args \
  --build-arg THINKSOFT_BUILD_VERSION="$THINKSOFT_BUILD_VERSION" \
