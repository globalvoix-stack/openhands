...
if [ "$(id -u)" -ne 0 ]; then
  echo "The Thinksoft entrypoint.sh must run as root"
  exit 1
fi
...
if [ -z "$WORKSPACE_MOUNT_PATH" ]; then
  # This is set to /opt/workspace in the Dockerfile. But if the user isn't mounting, we want to unset it so that Thinksoft doesn't mount at all
...
if [[ "$INSTALL_THIRD_PARTY_RUNTIMES" == "true" ]]; then
  ...
  if pip install 'thinksoft-ai[third_party_runtimes]' -qqq 2> >(tee /dev/stderr); then
...
if [[ "$SANDBOX_USER_ID" -eq 0 ]]; then
  echo "Running Thinksoft as root"
  export RUN_AS_THINKSOFT=false
  "$@"
else
  ...
  if id "enduser" &>/dev/null; then
    ...
    if ! useradd -l -m -u $SANDBOX_USER_ID -s /bin/bash enduser; then
      echo "Failed to create user enduser with id $SANDBOX_USER_ID. Moving thinksoft user."
      incremented_id=$(($SANDBOX_USER_ID + 1))
      usermod -u $incremented_id thinksoft
      if ! useradd -l -m -u $SANDBOX_USER_ID -s /bin/bash enduser; then
  ...
  usermod -aG thinksoft enduser
  # get the user group of /var/run/docker.sock and set thinksoft to that group
