...
Skills are specialized prompts that enhance Thinksoft with domain-specific knowledge and task-specific workflows. They help developers by providing expert guidance, automating common tasks, and ensuring consistent practices across projects. Each skill is designed to excel in a specific area, from Git operations to code review processes.

## Terminology Note

**Version 0 (V0)**: The term "microagents" continues to be used for V0 conversations. V0 is the current stable version of Thinksoft.
...
This directory (`Thinksoft/skills/`) contains shareable skills that will be used in V1 conversations. For V0 conversations, the system continues to use microagents from the same underlying files.

## Sources of Skills/Microagents

Thinksoft loads skills (V1) or microagents (V0) from two sources:

### 1. Shareable Skills/Microagents (Public)

This directory (`Thinksoft/skills/`) contains shareable skills (V1) or microagents (V0) that are:

- Available to all Thinksoft users
- Maintained in the Thinksoft repository
...
Each repository can have its own instructions in `.thinksoft/microagents/` (V0) or `.thinksoft/skills/` (V1). These instructions are:
...
- V1 supports both `.thinksoft/skills/` (preferred) and `.thinksoft/microagents/` (backward compatibility)
...
When Thinksoft works with a repository, it:

1. Loads repository-specific instructions from `.thinksoft/microagents/repo.md` (V0) or `.thinksoft/skills/` (V1) if present
2. Loads relevant knowledge agents based on keywords in conversations

**Note**: V1 also supports loading from `.thinksoft/microagents/` for backward compatibility.
...
You can see an example of a knowledge-based agent in [Thinksoft's github skill](https://github.com/Thinksoft/Thinksoft/tree/main/skills/github.md).

### 2. Repository Agents

Repository agents provide repository-specific knowledge and guidelines. They are:

- Loaded from `.thinksoft/microagents/repo.md` (V0) or `.thinksoft/skills/` directory (V1)
- V1 also supports `.thinksoft/microagents/` for backward compatibility
...
You can see an example of a repo agent in [the agent for the Thinksoft repo itself](https://github.com/Thinksoft/Thinksoft/blob/main/.thinksoft/microagents/repo.md).
...
2. **For Repository Agents**:
   ...
   - Consider using Thinksoft to generate a comprehensive repo.md (see [Creating a Repository Agent](#creating-a-repository-agent))
...
1. Create your agent file in the appropriate directory:
   ...
   - Note: Repository-specific agents should remain in their respective repositories' `.thinksoft/skills/` (V1) or `.thinksoft/microagents/` (V0) directory
2. Test thoroughly
3. Submit a pull request to Thinksoft

## License

All skills/microagents are subject to the same license as Thinksoft. See the root LICENSE file for details.
