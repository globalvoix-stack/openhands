#!/usr/bin/env python3
"""
Update Thinksoft OpenAPI documentation.
...
Behavior:
- Uses thinksoft.server.app.app.openapi() to build the spec.
- Preserves existing "servers" from docs/openapi.json if present; otherwise
  writes sensible defaults.
- Sets info.version to thinksoft.__version__.
...
os.environ['THINKSOFT_LOG_LEVEL'] = 'CRITICAL'
...
try:
    from thinksoft import __version__
    from thinksoft.server.app import app
except ImportError as e:
    print(f'Error importing Thinksoft modules: {e}')
    print(
