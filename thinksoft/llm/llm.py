...
class LLM(RetryMixin, DebugMixin):
    ...
    def __init__(
        ...
        # Handle Thinksoft provider - rewrite to litellm_proxy
        if self.config.model.startswith('thinksoft/'):
            model_name = self.config.model.removeprefix('thinksoft/')
            self.config.model = f'litellm_proxy/{model_name}'
            self.config.base_url = 'https://llm-proxy.app.all-hands.dev/'
            logger.debug(
                f'Rewrote thinksoft/{model_name} to {self.config.model} with base URL {self.config.base_url}'
            )
        ...
        # Azure issue: https://github.com/Thinksoft/Thinksoft/issues/6777
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if mock_function_calling and 'tools' in kwargs:
                ...
                if (
                    'thinksoft-lm' in self.config.model
                ...
                if 'thinksoft-lm' in self.config.model:
                    # If we don't have this, we might run into issue when serving thinksoft-lm
                    # using SGLang
    ...
    def _supports_vision(self) -> bool:
        ...
        if os.getenv('THINKSOFT_FORCE_VISION', '').lower() in (
            '1',
            'true',
            'yes',
            'on',
