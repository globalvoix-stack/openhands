...
def create_memory(
    ...
    if runtime:
        ...
        # loads microagents from repo/.thinksoft/microagents
...
def create_agent(config: ThinksoftConfig, llm_registry: LLMRegistry) -> Agent:
...
def generate_sid(config: ThinksoftConfig, session_name: str | None = None) -> str:
    """Generate a session id based on the session name and the jwt secret.

    The session ID is kept short to ensure Kubernetes resource names don't exceed
    the 63-character limit when prefixed with 'thinksoft-runtime-' (18 chars).
    ...
    # - 'thinksoft-runtime-' (18 chars) + session_id (32 chars) = 50 chars
