...
def log_uncaught_exceptions(
    ...
    thinksoft_logger.error('Uncaught exception', exc_info=(ex_cls, ex, tb))


sys.excepthook = log_uncaught_exceptions
thinksoft_logger = logging.getLogger('thinksoft')
current_log_level = logging.INFO

if LOG_LEVEL in logging.getLevelNamesMapping():
    current_log_level = logging.getLevelNamesMapping()[LOG_LEVEL]
thinksoft_logger.setLevel(current_log_level)

if DEBUG:
    thinksoft_logger.addFilter(StackInfoFilter())

if current_log_level == logging.DEBUG:
    thinksoft_logger.debug('DEBUG mode enabled.')

if LOG_JSON:
    thinksoft_logger.addHandler(json_log_handler(current_log_level))
    ...
    thinksoft_logger.addHandler(get_console_handler(current_log_level))

thinksoft_logger.addFilter(SensitiveDataFilter(thinksoft_logger.name))
thinksoft_logger.propagate = False
thinksoft_logger.debug('Logging initialized')

LOG_DIR = os.path.join(
    # parent dir of thinksoft/core (i.e., root of the repo)
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
    'logs',
)

if LOG_TO_FILE:
    thinksoft_logger.addHandler(
        get_file_handler(LOG_DIR, current_log_level)
    )  # default log to project root
    thinksoft_logger.debug(f'Logging to file in: {LOG_DIR}')
...
class LlmFileHandler(logging.FileHandler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        super().emit(record)
        self.stream.close()
        thinksoft_logger.debug('Logging to %s', self.baseFilename)
        self.message_counter += 1
...
class ThinksoftLoggerAdapter(logging.LoggerAdapter):
    extra: dict

    def __init__(
        self, logger: logging.Logger = thinksoft_logger, extra: dict | None = None
    ) -> None:
        self.logger = logger
        self.extra = extra or {}
