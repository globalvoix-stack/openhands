...
def load_from_env(
    cfg: ThinksoftConfig, env_or_toml_dict: dict | MutableMapping[str, str]
) -> None:
    ...
    Args:
        cfg: The ThinksoftConfig object to set attributes on.
...
def load_from_toml(cfg: ThinksoftConfig, toml_file: str = 'config.toml') -> None:
    """Load the config from the toml file. Supports both styles of config vars.

    Args:
        cfg: The ThinksoftConfig object to update attributes of.
    ...
    for key in toml_config:
        if key.lower() not in known_sections:
            logger.thinksoft_logger.warning(f'Unknown section [{key}] in {toml_file}')
...
def finalize_config(cfg: ThinksoftConfig) -> None:
...
def get_agent_config_arg(
    ...
    logger.thinksoft_logger.debug(f'Loading agent config from {agent_config_arg}')
    ...
    try:
        ...
        logger.thinksoft_logger.info(f'Config file not found: {e}')
        return None
    except toml.TomlDecodeError as e:
        logger.thinksoft_logger.error(
    ...
    logger.thinksoft_logger.debug(f'Loading from toml failed for {agent_config_arg}')
    return None
...
def get_llm_config_arg(
    ...
    if not os.path.exists(toml_file):
        logger.thinksoft_logger.debug(f'Config file not found: {toml_file}')
        return None
...
def get_condenser_config_arg(
    ...
    try:
        ...
        logger.thinksoft_logger.info(f'Config file not found: {toml_file}. Error: {e}')
        return None
    except toml.TomlDecodeError as e:
        logger.thinksoft_logger.error(
...
def parse_arguments() -> argparse.Namespace:
    ...
    if args.version:
        print(f'Thinksoft version: {get_version()}')
        sys.exit(0)

    return args


def register_custom_agents(config: ThinksoftConfig) -> None:
...
def setup_config_from_args(args: argparse.Namespace) -> ThinksoftConfig:
    ...
    3. ~/.thinksoft/settings.json and ~/.thinksoft/config.toml
    """
    # Load base config from toml and env vars
    config = load_thinksoft_config(config_file=args.config_file)

    # Override with command line arguments if provided
    if args.llm_config:
        logger.thinksoft_logger.debug(f'CLI specified LLM config: {args.llm_config}')
        ...
        if args.llm_config not in config.llms:
            ...
            if llm_config is None and args.config_file != os.path.join(
                os.path.expanduser('~'), '.thinksoft', 'config.toml'
            ):
