...
# Tag: Legacy-V0
"""Centralized command line argument configuration for Thinksoft CLI and headless modes."""
...
def get_cli_parser() -> argparse.ArgumentParser:
    ...
    description = (
        'Welcome to Thinksoft: Code Less, Make More\n\n'
        'Thinksoft supports two main commands:\n'
        '  serve - Launch the Thinksoft GUI server (web interface)\n'
        '  cli   - Run Thinksoft in CLI mode (terminal interface)\n\n'
        'Running "thinksoft" without a command is the same as "thinksoft cli"'
    )
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog='For more information about a command, run: thinksoft COMMAND --help',
    )
    ...
    subparsers = parser.add_subparsers(
        dest='command',
        title='commands',
        description='Thinksoft supports two main commands:',
        metavar='COMMAND',
    )

    # Add 'serve' subcommand
    serve_parser = subparsers.add_parser(
        'serve', help='Launch the Thinksoft GUI server using Docker (web interface)'
    )
    ...
    cli_parser = subparsers.add_parser(
        'cli', help='Run Thinksoft in CLI mode (terminal interface)'
    )
...
def get_evaluation_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description='Run Thinksoft in evaluation mode')
    add_common_arguments(parser)
    add_headless_specific_arguments(parser)
