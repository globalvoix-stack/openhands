...
from thinksoft.core.config import AgentConfig, ThinksoftConfig, SandboxConfig
...
class IssueResolver:
    ...
    def __init__(self, args: Namespace) -> None:
        ...
        self.app_config = self.update_thinksoft_config(
            load_thinksoft_config(),
            self.max_iterations,
            self.workspace_base,
    ...
    def update_sandbox_config(
        ...
        if (
            ...
            runtime_container_image = (
                f'ghcr.io/thinksoft/runtime:{thinksoft.__version__}-nikolaik'
            )
        ...
        thinksoft_config.sandbox.base_container_image = (
            sandbox_config.base_container_image
        )
        thinksoft_config.sandbox.runtime_container_image = (
            sandbox_config.runtime_container_image
        )
        thinksoft_config.sandbox.enable_auto_lint = sandbox_config.enable_auto_lint
        thinksoft_config.sandbox.use_host_network = sandbox_config.use_host_network
        thinksoft_config.sandbox.timeout = sandbox_config.timeout
        thinksoft_config.sandbox.local_runtime_url = sandbox_config.local_runtime_url
        thinksoft_config.sandbox.user_id = sandbox_config.user_id
    ...
    def initialize_runtime(
        ...
        logger.info('Checking for .thinksoft/setup.sh script...')
        runtime.maybe_run_setup_script()

        # Setup git hooks if they exist
        logger.info('Checking for .thinksoft/pre-commit.sh script...')
    ...
    async def resolve_issue(
        ...
        if self.repo_instruction is None:
            # Check for .thinksoft_instructions file in the workspace directory
            thinksoft_instructions_path = os.path.join(
                repo_dir, '.thinksoft_instructions'
            )
            if os.path.exists(thinksoft_instructions_path):
                with open(thinksoft_instructions_path, 'r') as f:
