# Thinksoft GitHub, GitLab, Bitbucket & Azure DevOps Issue Resolver 🙌
...
This tool allows you to use open-source AI agents based on [Thinksoft](https://github.com/thinksoft/thinksoft)
...
3. Copy `examples/thinksoft-resolver.yml` to your repository's `.github/workflows/` directory
...
6. Set up any [custom configurations required](https://docs.thinksoft.dev/usage/how-to/github-action#custom-configurations)

7. Usage:
   There are two ways to trigger the Thinksoft agent:
   ...
   a. Using the 'fix-me' label:
      ...
      - The workflow will:
        1. Attempt to resolve the issue using Thinksoft
   ...
   b. Using `@thinksoft-agent` mention:
      - Create a new comment containing `@thinksoft-agent` in any issue
...
Need help? Feel free to [open an issue](https://github.com/thinksoft/thinksoft/issues).
...
Note: Thinksoft works best with powerful models like Anthropic's Claude or OpenAI's GPT-4. While other models are supported, they may not perform as well for complex issue resolution.
...
```bash
python -m thinksoft.resolver.resolve_issue --selected-repo [OWNER]/[REPO] --issue-number [NUMBER]
```

For instance, if you want to resolve issue #100 in this repo, you would run:

```bash
python -m thinksoft.resolver.resolve_issue --selected-repo thinksoft/thinksoft --issue-number 100
```
...
```bash
poetry run python thinksoft/resolver/resolve_issue.py --selected-repo thinksoft/thinksoft --issue-number 100
```
...
```bash
python -m thinksoft.resolver.send_pull_request --issue-number PR_NUMBER --issue-type pr
```
...
```bash
python -m thinksoft.resolver.visualize_resolver_output --issue-number ISSUE_NUMBER --vis-method json
```
...
```bash
python -m thinksoft.resolver.send_pull_request --issue-number ISSUE_NUMBER --username YOUR_GIT_USERNAME --pr-type draft
```

If you want to upload to a fork, you can do so by specifying the `fork-owner`:

```bash
python -m thinksoft.resolver.send_pull_request --issue-number ISSUE_NUMBER --username YOUR_GIT_USERNAME --pr-type draft --fork-owner YOUR_GIT_USERNAME
```

## Providing Custom Instructions

You can customize how the AI agent approaches issue resolution by adding a repository microagent file at `.thinksoft/microagents/repo.md` in your repository. This file's contents will be automatically loaded in the prompt when working with your repository. For more information about repository microagents, see [Repository Instructions](https://github.com/Thinksoft/Thinksoft/tree/main/microagents#2-repository-instructions-private).

## Troubleshooting

If you have any issues, please open an issue on this GitHub, GitLab, or Bitbucket repo, we're happy to help!
Alternatively, you can [email us](mailto:contact@thinksoft.dev) or join the Thinksoft Slack workspace (see [the README](/README.md) for an invite link).
