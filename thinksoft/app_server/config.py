"""Configuration for the Thinksoft App Server."""
...
from thinksoft.sdk.utils.models import ThinksoftModel
...
def get_thinksoft_provider_base_url() -> str | None:
    """Return the base URL for the Thinksoft provider, if configured."""
    return os.getenv('THINKSOFT_PROVIDER_BASE_URL') or None


def _get_default_lifespan():
    # Check legacy parameters for saas mode. If we are in SAAS mode do not apply
    # Thinksoft alembic migrations
    if 'saas' in (os.getenv('THINKSOFT_CONFIG_CLS') or '').lower():
        return None
    return OssAppLifespanService()


class AppServerConfig(ThinksoftModel):
    persistence_dir: Path = Field(default_factory=get_default_persistence_dir)
    web_url: str | None = Field(
        default_factory=get_default_web_url,
        description='The URL where Thinksoft is running (e.g., http://localhost:3000)',
    )
    thinksoft_provider_base_url: str | None = Field(
        default_factory=get_thinksoft_provider_base_url,
        description='Base URL for the Thinksoft provider',
    )
