...
class AppConversationServiceBase(AppConversationService, ABC):
    ...
    async def load_and_merge_all_skills(
        ...
        try:
            ...
            # Load user skills from ~/.thinksoft/skills/ directory
            # Uses the SDK's load_user_skills() function which handles loading from
            # ~/.thinksoft/skills/ and ~/.thinksoft/microagents/ (for backward compatibility)
            try:
    ...
    async def clone_or_init_git_repo(
        ...
        if request.selected_branch:
            ...
            checkout_command = f'git checkout -b {thinksoft_workspace_branch}'
    ...
    async def maybe_run_setup_script(
        ...
        setup_script = workspace.working_dir + '/.thinksoft/setup.sh'
    ...
    async def maybe_setup_git_hooks(
        ...
        command = 'mkdir -p .git/hooks && chmod +x .thinksoft/pre-commit.sh'
        ...
        with tempfile.TemporaryFile(mode='w+t') as temp_file:
            ...
            if result.get('success'):
                ...
                if 'This hook was installed by Thinksoft' not in temp_file.read():
