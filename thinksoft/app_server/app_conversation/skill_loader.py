...
- Global skills from Thinksoft/skills/
- User skills from ~/.thinksoft/skills/
...
GLOBAL_SKILLS_DIR = os.path.join(
    os.path.dirname(os.path.dirname(thinksoft.__file__)),
    'skills',
)
...
def load_global_skills() -> list[Skill]:
    """Load global skills from Thinksoft/skills/ directory.
...
async def _is_gitlab_repository(repo_name: str, user_context: UserContext) -> bool:
    ...
    try:
        ...
        # This is a safe fallback since we'll just use the default .thinksoft
        return False
...
async def _determine_org_repo_path(
    ...
    Examples:
        - GitHub/Bitbucket: ('owner/.thinksoft', 'owner')
        - GitLab: ('owner/thinksoft-config', 'owner')
        - Azure DevOps: ('org/thinksoft-config/thinksoft-config', 'org')
    """
    ...
    # For GitLab and Azure DevOps, use thinksoft-config (since .thinksoft is not a valid repo name)
    # For other providers, use .thinksoft
    if is_gitlab:
        org_thinksoft_repo = f'{org_name}/thinksoft-config'
    elif is_azure_devops:
        # Azure DevOps format: org/project/repo
        # For org-level config, use: org/thinksoft-config/thinksoft-config
        org_thinksoft_repo = f'{org_name}/thinksoft-config/thinksoft-config'
    else:
        org_thinksoft_repo = f'{org_name}/.thinksoft'

    return org_thinksoft_repo, org_name
...
def _merge_repo_skills_with_precedence(
    ...
    2. .thinksoft/skills/ directory
    3. .thinksoft/microagents/ directory (backward compatibility)

    Args:
        special_skills: Skills from special files in repo root
        skills_dir_skills: Skills from .thinksoft/skills/ directory
        microagents_dir_skills: Skills from .thinksoft/microagents/ directory
...
async def load_repo_skills(
    ...
    2. .md files in .thinksoft/skills/ directory (preferred)
    3. .md files in .thinksoft/microagents/ directory (for backward compatibility)
    ...
    try:
        ...
        # Load .md files from .thinksoft/skills/ directory (preferred)
        skills_dir = f'{repo_root}/.thinksoft/skills'
        skills_dir_skills = await _find_and_load_skill_md_files(
            workspace, skills_dir, working_dir
        )

        # Load .md files from .thinksoft/microagents/ directory (backward compatibility)
        microagents_dir = f'{repo_root}/.thinksoft/microagents'
...
async def _get_org_repository_url(
    org_thinksoft_repo: str, user_context: UserContext
) -> str | None:
    """Get authenticated Git URL for organization repository.

    Args:
        org_thinksoft_repo: Organization repository path
    ...
    try:
        remote_url = await user_context.get_authenticated_git_url(
            org_thinksoft_repo, is_optional=True
        )
        return remote_url
    except AuthenticationError as e:
        _logger.debug(
            f'org-level skill directory {org_thinksoft_repo} not found: {str(e)}'
        )
        return None
    except Exception as e:
        _logger.debug(
            f'Failed to get authenticated URL for {org_thinksoft_repo}: {str(e)}'
        )
        return None
...
async def _clone_org_repository(
    ...
    Args:
        ...
        working_dir: Working directory for command execution
        org_thinksoft_repo: Organization repository path (for logging)
    ...
    if result.exit_code != 0:
        _logger.info(
            f'No org-level skills found at {org_thinksoft_repo} (exit_code: {result.exit_code})'
        )
        _logger.debug(f'Clone command output: {result.stderr}')
        return False

    _logger.info(f'Successfully cloned org-level skills from {org_thinksoft_repo}')
    return True
...
async def load_org_skills(
    ...
    github.com/acme-co/.thinksoft exists. If it does, it will clone it and load
    the skills from both the ./skills/ and ./microagents/ folders.

    For GitLab repositories, it will use thinksoft-config instead of .thinksoft
    since GitLab doesn't support repository names starting with non-alphanumeric
    characters.

    For Azure DevOps repositories, it will use org/thinksoft-config/thinksoft-config
    ...
    try:
        ...
        org_thinksoft_repo, org_name = await _determine_org_repo_path(
            selected_repository, user_context
        )

        _logger.info(f'Checking for org-level skills at {org_thinksoft_repo}')

        # Get authenticated URL for org repository
        remote_url = await _get_org_repository_url(org_thinksoft_repo, user_context)
        if not remote_url:
            return []

        # Clone the organization repository
        org_repo_dir = f'{working_dir}/_org_thinksoft_{org_name}'
        clone_success = await _clone_org_repository(
            workspace, remote_url, org_repo_dir, working_dir, org_thinksoft_repo
        )
        ...
        _logger.info(
            f'Loaded {len(loaded_skills)} skills from org-level repository {org_thinksoft_repo}: {[s.name for s in loaded_skills]}'
        )
