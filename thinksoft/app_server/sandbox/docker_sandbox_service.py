...
class DockerSandboxService(SandboxService):
    ...
    async def delete_sandbox(self, sandbox_id: str) -> bool:
        ...
        try:
            ...
            try:
                volume_name = f'thinksoft-workspace-{sandbox_id}'
...
class DockerSandboxServiceInjector(SandboxServiceInjector):
    ...
    host_port: int = Field(
        default=3000,
        description=(
            'The port on which the main Thinksoft app server is running. '
            'Used for webhook callbacks from agent-server containers. '
            'If running Thinksoft on a non-default port, set this to match. '
