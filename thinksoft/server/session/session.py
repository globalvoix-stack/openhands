...
from thinksoft.core.logger import ThinksoftLoggerAdapter
...
class WebSession:
    ...
    Attributes:
        ...
        config: Effective Thinksoft configuration for this conversation.
    ...
    def __init__(
        ...
        self.logger = ThinksoftLoggerAdapter(extra={'session_id': sid})
    ...
    async def initialize_agent(
        ...
        # Add Thinksoft' MCP server by default
        thinksoft_mcp_server, thinksoft_mcp_stdio_servers = (
            ThinksoftMCPConfigImpl.create_default_mcp_server_config(
                self.config.mcp_host, self.config, self.user_id
            )
        )

        if thinksoft_mcp_server:
            self.config.mcp.shttp_servers.append(thinksoft_mcp_server)
            self.logger.debug('Added default MCP HTTP server to config')

            self.config.mcp.stdio_servers.extend(thinksoft_mcp_stdio_servers)
