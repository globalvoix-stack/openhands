...
class DockerNestedConversationManager(ConversationManager):
    ...
    async def get_running_agent_loops(
        ...
        conversation_ids = {
            name[len('thinksoft-runtime-') :]
            for name in names
            if name.startswith('thinksoft-runtime-')
        }
    ...
    async def close_session(self, sid: str):
        ...
        try:
            container = self.docker_client.containers.get(f'thinksoft-runtime-{sid}')
    ...
    async def get_agent_loop_info(
        ...
        for container in containers:
            if not container.name or not container.name.startswith(
                ...
                continue
            conversation_id = container.name[len('thinksoft-runtime-') :]
    ...
    def get_nested_url_for_container(self, container: Container) -> str:
        ...
        conversation_id = container_name[len('thinksoft-runtime-') :]
    ...
    async def _create_runtime(
        ...
        env_vars['USER'] = 'thinksoft' if config.run_as_thinksoft else 'root'
        ...
        if config.file_store == 'local':
            ...
            volumes.append(
                f'{file_store_path}/{conversation_dir}:/root/.thinksoft/{conversation_dir}:rw'
            )
