...
class SuggestedTask(BaseModel):
    ...
    def get_prompt_for_task(
        ...
        env = Environment(
            loader=FileSystemLoader('thinksoft/integrations/templates/suggested_task')
        )
...
class BaseGitService(ABC):
    ...
    def _determine_microagents_path(self, repository_name: str) -> str:
        ...
        if actual_repo_name == '.thinksoft' or actual_repo_name == 'thinksoft-config':
            # For repository name ".thinksoft", scan "microagents" folder
            return 'microagents'
        else:
            # Default behavior: look for .thinksoft/microagents directory
            return '.thinksoft/microagents'
