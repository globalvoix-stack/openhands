# Thinksoft Kubernetes Runtime

This directory contains the Kubernetes runtime implementation for Thinksoft, which allows the software to run on Kubernetes clusters for scalable and isolated execution environments.

## Local Development with KIND

For local development and testing, Thinksoft provides a convenient setup using KIND (Kubernetes IN Docker) that creates a local Kubernetes cluster.
...
To use the Kubernetes runtime, you need to configure Thinksoft properly. The configuration is done through a TOML configuration file.
...
2. **Runtime Container Image**: Specify the container image to use for the runtime environment
   ```toml
   [sandbox]
   runtime_container_image = "docker.thinksoft.dev/thinksoft/runtime:1.2-nikolaik"
   ```

#### Additional Kubernetes Options

Thinksoft provides extensive configuration options for Kubernetes deployments under the `[kubernetes]` section. These options allow you to customize:
...
To set up and run Thinksoft with the Kubernetes runtime locally:
...
2. Automatically start the Thinksoft application using mirrord
