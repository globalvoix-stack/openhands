...
def check_dependencies(code_repo_path: str, check_browser: bool) -> None:
    ERROR_MESSAGE = 'Please follow the instructions in https://github.com/Thinksoft/Thinksoft/blob/main/Development.md to install Thinksoft.'
...
class LocalRuntime(ActionExecutionClient):
    ...
    def __init__(
        ...
        logger.warning(
            ...
            'This is an experimental feature, please report issues to https://github.com/Thinksoft/Thinksoft/issues. '
            '`run_as_thinksoft` will be ignored since the current user will be used to launch the server. '
        ...
        if self.sid in _RUNNING_SERVERS:
            ...
            if self.config.workspace_base is not None:
                ...
                self._temp_workspace = tempfile.mkdtemp(
                    prefix=f'thinksoft_workspace_{self.sid}',
                )
    ...
    def setup(cls, config: ThinksoftConfig, headless_mode: bool = False):
        ...
        if should_check_dependencies:
            code_repo_path = os.path.dirname(os.path.dirname(thinksoft.__file__))
...
def _create_server(
    ...
    temp_workspace = tempfile.mkdtemp(
        prefix=f'thinksoft_workspace_{workspace_prefix}',
    )
    ...
    env['THINKSOFT_REPO_PATH'] = code_repo_path
...
def _get_plugins(config: ThinksoftConfig) -> list[PluginRequirement]:
