...
class DockerRuntimeBuilder(RuntimeBuilder):
    ...
    def build(
        ...
        if not DockerRuntimeBuilder.check_buildx(self.is_podman):
            # when running thinksoft in a container, there might not be a "docker"
            # binary available, in which case we need to download docker binary.
            # since the official thinksoft app image is built from debian, we use
            # debian way to install docker binary
            logger.info(
                'No docker binary available inside thinksoft-app container, trying to download online...'
            )
            commands = [
                'apt-get update',
        ...
        buildx_cmd = [
            ...
            'build',
            '--progress=plain',
            f'--build-arg=THINKSOFT_RUNTIME_VERSION={get_version()}',
            f'--build-arg=THINKSOFT_RUNTIME_BUILD_TIME={datetime.datetime.now().isoformat()}',
