...
class VSCodePlugin(Plugin):
    ...
    async def initialize(self, username: str, runtime_id: str | None = None) -> None:
        ...
        if username not in filter(None, [RUNTIME_USERNAME, 'root', 'thinksoft']):
            self.vscode_port = None
            self.vscode_connection_token = None
            logger.warning(
                'VSCodePlugin is only supported for root or thinksoft user. '
        ...
        if explicit_base:
            ...
            cmd = (
                ...
                + f'sudo chown -R {username}:{username} /thinksoft/.openvscode-server\n'
                + f'cd {workspace_path}\n'
                + 'exec /thinksoft/.openvscode-server/bin/openvscode-server '
