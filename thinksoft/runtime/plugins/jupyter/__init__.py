...
class JupyterPlugin(Plugin):
    ...
    async def initialize(
        self, username: str, kernel_id: str = 'thinksoft-default'
    ) -> None:
        ...
        if not is_local_runtime:
            ...
            poetry_prefix = (
                'cd /thinksoft/code\n'
                'export POETRY_VIRTUALENVS_PATH=/thinksoft/poetry;\n'
                'export PYTHONPATH=/thinksoft/code:$PYTHONPATH;\n'
                'export MAMBA_ROOT_PREFIX=/thinksoft/micromamba;\n'
                '/thinksoft/micromamba/bin/micromamba run -n thinksoft '
            )
        else:
            # LocalRuntime
            prefix = ''
            code_repo_path = os.environ.get('THINKSOFT_REPO_PATH')
            if not code_repo_path:
                raise ValueError(
                    'THINKSOFT_REPO_PATH environment variable is not set. '
