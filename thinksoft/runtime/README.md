...
The Thinksoft Runtime folder contains the core components responsible for executing actions and managing the runtime environment for the Thinksoft project. This README provides an overview of the main components and their interactions.
You can learn more about how the runtime works in the [Docker Runtime](https://docs.thinksoft.dev/usage/architecture/runtime) documentation.
...
6. **Sandbox Environment**:
   ...
   - Actions received from the Thinksoft backend are executed in this sandboxed environment.
...
This is the default runtime used within Thinksoft.
...
**Important: This runtime provides no isolation as it runs directly on the host machine. All actions are executed with the same permissions as the user running Thinksoft. For secure execution with proper isolation, use the Docker Runtime instead.**
...
At the time of this writing, this is mostly used in parallel evaluation, such as this example for [SWE-Bench](https://github.com/Thinksoft/Thinksoft/tree/main/evaluation/benchmarks/swe_bench#run-inference-on-remoteruntime-experimental).

## Related Components

- The runtime interacts closely with the event system defined in the `thinksoft.events` module.
- It relies on configuration classes from `thinksoft.core.config`.
- Logging is handled through `thinksoft.core.logger`.

This section provides an overview of the Thinksoft Runtime folder. For more detailed information on specific components or usage, please refer to the individual files and their docstrings.
