...
class Runtime(FileEditRuntimeMixin):
    ...
    async def clone_or_init_repo(
        ...
        thinksoft_workspace_branch = f'thinksoft-workspace-{random_str}'
        ...
        checkout_command = (
            ...
            if selected_branch
            else f'git checkout -b {thinksoft_workspace_branch}'
        )
    ...
    def maybe_run_setup_script(self):
        """Run .thinksoft/setup.sh if it exists in the workspace or repository."""
    ...
    def maybe_setup_git_hooks(self):
        """Set up git hooks if .thinksoft/pre-commit.sh exists in the workspace or repository."""
        pre_commit_script = '.thinksoft/pre-commit.sh'
        ...
        if not isinstance(read_obs, ErrorObservation):
            # If the existing hook wasn't created by Thinksoft, preserve it
            if 'This hook was installed by Thinksoft' not in read_obs.content:
...
"""
    ...
    def get_microagents_from_org_or_user(
        ...
        self.log(
            'info',
            f'Checking for org-level microagents at {org_thinksoft_repo}',
        )

        # Try to clone the org-level repo
        try:
            # Create a temporary directory for the org-level repo
            org_repo_dir = self.workspace_root / f'org_thinksoft_{org_name}'
            self.log(
                'debug',
            ...
            try:
                ...
                self.log(
                    'debug',
                    f'org-level microagent directory {org_thinksoft_repo} not found: {str(e)}',
                )
                raise
            except Exception as e:
                self.log(
            ...
            if isinstance(obs, CmdOutputObservation) and obs.exit_code == 0:
                self.log(
                    'info',
                    f'Successfully cloned org-level microagents from {org_thinksoft_repo}',
                )
                ...
                self.log(
                    'info',
                    f'Loaded {len(loaded_microagents)} microagents from org-level repository {org_thinksoft_repo}',
                )
                ...
                self.log(
                    'info',
                    f'No org-level microagents found at {org_thinksoft_repo} (exit_code: {exit_code})',
                )
                self.log(
                    'debug',
            ...
            self.log(
                'debug',
                f'Error loading org-level microagents from {org_thinksoft_repo}: {str(e)}',
            )
    ...
    def get_microagents_from_selected_repo(
        ...
        github.com/acme-co/.thinksoft and load microagents from there if it exists.
        ...
        """
        loaded_microagents: list[BaseMicroagent] = []
        microagents_dir = self.workspace_root / '.thinksoft' / 'microagents'
        repo_root = None
        ...
        if selected_repository:
            ...
            microagents_dir = repo_root / '.thinksoft' / 'microagents'
        ...
        # Check for legacy .thinksoft_instructions file
        obs = self.read(
            FileReadAction(path=str(self.workspace_root / '.thinksoft_instructions'))
        )
        if isinstance(obs, ErrorObservation) and repo_root is not None:
            ...
            self.log(
                'debug',
                f'.thinksoft_instructions not present, trying to load from repository microagents_dir={microagents_dir}',
            )
            obs = self.read(
                FileReadAction(path=str(repo_root / '.thinksoft_instructions'))
            )

        if isinstance(obs, FileReadObservation):
            self.log('info', 'thinksoft_instructions microagent loaded.')
            loaded_microagents.append(
                BaseMicroagent.load(
                    path='.thinksoft_instructions',
                    microagent_dir=None,
    ...
    @classmethod
    def setup(cls, config: ThinksoftConfig, headless_mode: bool = False):
        """Set up the environment for runtimes to be created."""

    @classmethod
    def teardown(cls, config: ThinksoftConfig):
