...
DEFAULT_PYTHON_PREFIX = [
    '/thinksoft/micromamba/bin/micromamba',
    'run',
    '-n',
    'thinksoft',
    'poetry',
...
def get_action_execution_server_startup_command(
    ...
    username = (
        override_username
        or RUNTIME_USERNAME
        or ('thinksoft' if app_config.run_as_thinksoft else 'root')
    )
    user_id = (
        override_user_id or RUNTIME_UID or (1000 if app_config.run_as_thinksoft else 0)
    )
