...
def get_runtime_image_repo() -> str:
    return os.getenv('OH_RUNTIME_RUNTIME_IMAGE_REPO', 'ghcr.io/thinksoft/runtime')
...
def build_runtime_image(
    ...
    If dry_run is False, it will also build the Thinksoft runtime Docker image using the docker build folder.
...
def prep_build_folder(
    ...
    thinksoft_source_dir = Path(thinksoft.__file__).parent
    project_root = thinksoft_source_dir.parent
    logger.debug(f'Building source distribution using project root: {project_root}')

    # Copy the 'thinksoft' directory (Source code)
    shutil.copytree(
        thinksoft_source_dir,
        Path(build_folder, 'code', 'thinksoft'),
    ...
    for file in ['pyproject.toml', 'poetry.lock']:
        src = Path(thinksoft_source_dir, file)
        if not src.exists():
...
def get_hash_for_lock_files(base_image: str, enable_browser: bool = True) -> str:
    ...
    for file in ['pyproject.toml', 'poetry.lock']:
        src = Path(thinksoft_source_dir, file)
        if not src.exists():
            src = Path(thinksoft_source_dir.parent, file)
