...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('thinksoft_helped_author', sa.Boolean(), nullable=True),
        sa.Column('num_thinksoft_commits', sa.Integer(), nullable=True),
        sa.Column('num_thinksoft_review_comments', sa.Integer(), nullable=True),
        sa.Column('num_thinksoft_general_comments', sa.Integer(), nullable=True),
    )

    # Create indexes for efficient querying
    op.create_index(
        'ix_thinksoft_prs_repo_id', 'thinksoft_prs', ['repo_id'], unique=False
    )
    op.create_index(
        'ix_thinksoft_prs_pr_number', 'thinksoft_prs', ['pr_number'], unique=False
    )
    op.create_index(
        'ix_thinksoft_prs_status', 'thinksoft_prs', ['status'], unique=False
    )

    # Create unique constraint on repo_id + pr_number combination
    op.create_index(
        'ix_thinksoft_prs_repo_pr_unique',
        'thinksoft_prs',
        ['repo_id', 'pr_number'],
        unique=True,
    )
...
def downgrade() -> None:
    op.drop_index('ix_thinksoft_prs_repo_id', table_name='thinksoft_prs')
    op.drop_index('ix_thinksoft_prs_pr_number', table_name='thinksoft_prs')
    op.drop_index('ix_thinksoft_prs_status', table_name='thinksoft_prs')
    op.drop_index('ix_thinksoft_prs_repo_pr_unique', table_name='thinksoft_prs')
    op.drop_table('thinksoft_prs')
