# Thinksoft Enterprise Usage Telemetry Service
...
5. [Implementation Plan](#5-implementation-plan)
   - 5.1 [Database Schema and Models (M1)](#51-database-schema-and-models-m1)
     - 5.1.1 [Thinksoft - Database Migration](#511-thinksoft---database-migration)
     - 5.1.2 [Thinksoft - Model Tests](#512-thinksoft---model-tests)
   - 5.2 [Metrics Collection Framework (M2)](#52-metrics-collection-framework-m2)
     - 5.2.1 [Thinksoft - Core Collection Framework](#521-thinksoft---core-collection-framework)
     - 5.2.2 [Thinksoft - Example Collectors](#522-thinksoft---example-collectors)
     - 5.2.3 [Thinksoft - Framework Tests](#523-thinksoft---framework-tests)
   - 5.3 [Collection and Upload Processors (M3)](#53-collection-and-upload-processors-m3)
     - 5.3.1 [Thinksoft - Collection Processor](#531-thinksoft---collection-processor)
     - 5.3.2 [Thinksoft - Upload Processor](#532-thinksoft---upload-processor)
     - 5.3.3 [Thinksoft - Integration Tests](#533-thinksoft---integration-tests)
   - 5.4 [License Warning API (M4)](#54-license-warning-api-m4)
     - 5.4.1 [Thinksoft - License Status API](#541-thinksoft---license-status-api)
     - 5.4.2 [Thinksoft - API Integration](#542-thinksoft---api-integration)
   - 5.5 [UI Warning Banner (M5)](#55-ui-warning-banner-m5)
     - 5.5.1 [Thinksoft - UI Warning Banner](#551-thinksoft---ui-warning-banner)
     - 5.5.2 [Thinksoft - UI Integration](#552-thinksoft---ui-integration)
   - 5.6 [Helm Chart Deployment Configuration (M6)](#56-helm-chart-deployment-configuration-m6)
     - 5.6.1 [Thinksoft-Cloud - Cronjob Manifests](#561-thinksoft-cloud---cronjob-manifests)
     - 5.6.2 [Thinksoft-Cloud - Configuration Management](#562-thinksoft-cloud---configuration-management)
   - 5.7 [Documentation and Enhanced Collectors (M7)](#57-documentation-and-enhanced-collectors-m7)
     - 5.7.1 [Thinksoft - Advanced Collectors](#571-thinksoft---advanced-collectors)
     - 5.7.2 [Thinksoft - Monitoring and Testing](#572-thinksoft---monitoring-and-testing)
     - 5.7.3 [Thinksoft - Technical Documentation](#573-thinksoft---technical-documentation)

## 1. Introduction

### 1.1 Problem Statement

Thinksoft Enterprise (OHE) helm charts are publicly available but not open source, creating a visibility gap for the sales team. Unknown users can install and use OHE without the vendor's knowledge, preventing proper customer engagement and sales pipeline management. Without usage telemetry, the vendor cannot identify potential customers, track installation health, or proactively support users who may need assistance.
...
When telemetry uploads fail for more than 4 days, users will see a prominent warning banner in the Thinksoft Enterprise UI:

```
⚠️ Your Thinksoft Enterprise license will expire in 30 days. Please contact support if this issue persists.
```
...
2. **Generates unique customer identifiers** using administrator contact information:
   - Customer email: Determined by the following priority order:
     1. `THINKSOFT_ADMIN_EMAIL` environment variable (if set in helm values)
...
**Tier 1: Explicit Configuration** - The `THINKSOFT_ADMIN_EMAIL` environment variable allows administrators to explicitly specify the contact email during deployment.
...
class TelemetryUploadProcessor(MaintenanceTaskProcessor):
    ...
    def _get_admin_email(self) -> str | None:
        ...
        env_admin_email = os.getenv('THINKSOFT_ADMIN_EMAIL')
        if env_admin_email:
...
async def get_license_status():
    ...
    if not _is_thinksoft_enterprise():
    ...
    with session_maker() as session:
        ...
        if not last_upload:
            ...
            return {
                "warn": True,
                "message": "Thinksoft Enterprise license verification pending. Please ensure network connectivity."
            }
        ...
        if days_since_upload > 4:
            ...
            if oldest_unsent:
                ...
                if days_until_expiration <= 0:
                    message = "Your Thinksoft Enterprise license has expired. Please contact support immediately."
                else:
                    message = f"Your Thinksoft Enterprise license will expire in {days_until_expiration} days. Please contact support if this issue persists."

                return {"warn": True, "message": message}

        return {"warn": False, "message": ""}

def _is_thinksoft_enterprise() -> bool:
...
The cronjob configurations will be deployed via the Thinksoft-Cloud helm charts.
...
```yaml
# charts/thinksoft/templates/telemetry-collection-cronjob.yaml
apiVersion: batch/v1
kind: CronJob
metadata:
  name: {{ include "thinksoft.fullname" . }}-telemetry-collection
  labels:
    {{- include "thinksoft.labels" . | nindent 4 }}
spec:
      ...
      template:
        spec:
          ...
          - name: telemetry-collector
            ...
            env:
            {{- include "thinksoft.env" . | nindent 12 }}
            command:
            - python
            - -c
            - |
...
```yaml
# charts/thinksoft/templates/telemetry-upload-cronjob.yaml
apiVersion: batch/v1
kind: CronJob
metadata:
  name: {{ include "thinksoft.fullname" . }}-telemetry-upload
  labels:
    {{- include "thinksoft.labels" . | nindent 4 }}
spec:
        ...
        spec:
          ...
          - name: telemetry-uploader
            ...
            - name: REPLICATED_PUBLISHABLE_KEY
              valueFrom:
                secretKeyRef:
                  name: {{ include "thinksoft.fullname" . }}-replicated-config
                  key: publishable-key
            - name: REPLICATED_APP_SLUG
              value: {{ .Values.telemetry.replicatedAppSlug | default "thinksoft-enterprise" | quote }}
            command:
            - python
            - -c
            - |
              ...
              processor = TelemetryUploadProcessor(
                  ...
                  replicated_app_slug=os.getenv('REPLICATED_APP_SLUG', 'thinksoft-enterprise')
              )
              task = MaintenanceTask()
              task.set_processor(processor)
              task.status = MaintenanceTaskStatus.PENDING
...
#### 5.2.1 Thinksoft - Core Collection Framework
...
#### 5.2.2 Thinksoft - Example Collectors
...
#### 5.2.3 Thinksoft - Framework Tests
...
#### 5.5.1 Thinksoft - UI Warning Banner
...
#### 5.5.2 Thinksoft - UI Integration
...
**Repository**: Thinksoft-Cloud
Create Kubernetes cronjob configurations and deployment scripts.

#### 5.6.1 Thinksoft-Cloud - Cronjob Manifests

- [ ] `charts/thinksoft/templates/telemetry-collection-cronjob.yaml`
- [ ] `charts/thinksoft/templates/telemetry-upload-cronjob.yaml`

#### 5.6.2 Thinksoft-Cloud - Configuration Management

- [ ] `charts/thinksoft/templates/replicated-secret.yaml`
- [ ] Update `charts/thinksoft/values.yaml` with telemetry configuration options:
  ```yaml
  # Add to values.yaml
  telemetry:
    enabled: true
    replicatedAppSlug: "thinksoft-enterprise"
    adminEmail: ""  # Optional: admin email for customer identification

  # Add to deployment environment variables
  env:
    THINKSOFT_ADMIN_EMAIL: "{{ .Values.telemetry.adminEmail }}"
  ```
...
#### 5.7.1 Thinksoft - Advanced Collectors
...
#### 5.7.2 Thinksoft - Monitoring and Testing
...
#### 5.7.3 Thinksoft - Technical Documentation
