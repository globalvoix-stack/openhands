...
from thinksoft.server.routes.git import (
...
async def saas_get_repository_microagents(
    ...
    - If git provider is not GitLab and actual repository name is ".thinksoft": scans "microagents" folder
    - If git provider is GitLab and actual repository name is "thinksoft-config": scans "microagents" folder
    - Otherwise: scans ".thinksoft/microagents" folder
...
async def saas_get_repository_microagent_content(
    ...
    Example:
        GET /api/user/repository/owner/repo/microagents/content?file_path=.thinksoft/microagents/my-agent.md
    """
    if not provider_tokens:
        retval = await _check_idp(
            access_token=access_token,
