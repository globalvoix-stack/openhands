...
from thinksoft.events.action import FileEditAction, FileReadAction
...
def test_edit_from_scratch(temp_dir, runtime_cls, run_as_thinksoft):
...
def test_edit(temp_dir, runtime_cls, run_as_thinksoft):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_thinksoft)
    try:
        action = FileEditAction(
            content=ORGINAL,
...
def test_edit_long_file(temp_dir, runtime_cls, run_as_thinksoft):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_thinksoft)
    try:
        action = FileEditAction(
