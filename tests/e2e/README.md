# Thinksoft End-to-End Tests

This directory contains end-to-end tests for the Thinksoft application. These tests use Playwright to interact with the Thinksoft UI and verify that the application works correctly.
...
- `--base-url`: Specify the base URL of the Thinksoft instance under test (default: `http://localhost:12000`)
...
By default, the tests run against `http://localhost:12000`. You can specify a different Thinksoft instance URL using the `--base-url` option:

```bash
cd tests/e2e
# Run against a remote instance
poetry run pytest test_settings.py::test_github_token_configuration test_conversation.py::test_conversation_start -v --base-url=https://my-thinksoft-instance.com
...
1. Navigates to the Thinksoft application
...
1. Navigates to the Thinksoft application (assumes GitHub token is already configured)
2. Selects the "thinksoft-agent/Thinksoft" repository
...
A simple test (`test_simple_browser_navigation`) that just navigates to the Thinksoft GitHub repository to verify the browser setup works correctly.
...
2. Check the logs in `/tmp/thinksoft-e2e-test.log` and `/tmp/thinksoft-e2e-build.log`
3. Verify that the Thinksoft application is running correctly
