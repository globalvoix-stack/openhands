...
def mock_agent():
    ...
    agent.llm.config = ThinksoftConfig().get_llm_config()
...
async def test_memory_on_event_exception_handling(memory, event_stream, mock_agent):
    ...
    with (
        ...
        patch('thinksoft.core.main.create_agent', return_value=mock_agent),
    ):
...
"""
    ...
    with patch('thinksoft.memory.memory.GLOBAL_MICROAGENTS_DIR', test_microagents_dir):
        # Initialize Memory
        memory = Memory(
            event_stream=event_stream,
            sid='test-session',
...
async def test_conversation_instructions_plumbed_to_memory(
    ...
    with (
        patch(
            'thinksoft.server.session.agent_session.AgentController', SpyAgentController
        ),
    ):
        await session.start(
