...
from thinksoft.core.exceptions import LLMNoResponseError, OperationCancelled
...
def mock_logger(monkeypatch):
    ...
    monkeypatch.setattr('thinksoft.llm.debug_mixin.llm_prompt_logger', mock_logger)
    monkeypatch.setattr('thinksoft.llm.debug_mixin.llm_response_logger', mock_logger)
    monkeypatch.setattr('thinksoft.llm.llm.logger', mock_logger)
    return mock_logger
...
@patch('thinksoft.llm.llm.litellm.get_model_info')
...
@patch('thinksoft.llm.llm.litellm_completion')
...
@patch('thinksoft.llm.llm.litellm.get_model_info')
@patch('thinksoft.llm.llm.httpx.get')
...
@patch('thinksoft.llm.llm.litellm.token_counter')
...
@patch('thinksoft.llm.llm.litellm.token_counter')
@patch('thinksoft.llm.llm.create_pretrained_tokenizer')
...
@patch('thinksoft.llm.llm.httpx.get')
@patch('thinksoft.llm.llm.litellm.get_model_info')
def test_thinksoft_provider_rewrite_and_caching_prompt(
    ...
    config = LLMConfig(
        model='thinksoft/claude-3.7-sonnet', api_key='k', caching_prompt=True
    )
    llm = LLM(config, service_id='svc')
...
@patch('thinksoft.llm.async_llm.litellm_acompletion')
@pytest.mark.asyncio
...
@patch('thinksoft.llm.streaming_llm.AsyncLLM._call_acompletion')
@pytest.mark.asyncio
