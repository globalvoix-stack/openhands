...
def temp_config_files(tmp_path):
    ...
    # Create a directory structure mimicking ~/.thinksoft/
    user_config_dir = tmp_path / 'home' / '.thinksoft'
    user_config_dir.mkdir(parents=True, exist_ok=True)

    # Create ~/.thinksoft/config.toml
...
""")
    ...
    # Create ~/.thinksoft/settings.json
...
@patch('thinksoft.core.config.utils.os.path.expanduser')
...
def test_current_dir_toml_precedence_over_user_config(
    ...
    """Test that config.toml in current directory has precedence over ~/.thinksoft/config.toml."""
    mock_expanduser.side_effect = lambda path: path.replace(
        '~', temp_config_files['home_dir']
    )
...
def test_default_values_applied_when_none():
    ...
    # Load config
    with patch(
        'thinksoft.core.config.utils.load_thinksoft_config',
        return_value=ThinksoftConfig(),
    ):
        config = setup_config_from_args(mock_args)
...
def test_cli_args_none_uses_config_toml_values():
    ...
    config_from_toml = ThinksoftConfig()
