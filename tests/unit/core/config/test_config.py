...
def default_config(monkeypatch):
    # Fixture to provide a default ThinksoftConfig instance
    yield ThinksoftConfig()


def test_compat_env_to_config(monkeypatch, setup_env):
    # Use `monkeypatch` to set environment variables for this specific test
    monkeypatch.setenv('SANDBOX_VOLUMES', '/repos/thinksoft/workspace:/workspace:rw')
    ...
    assert config.sandbox.volumes == '/repos/thinksoft/workspace:/workspace:rw'
    # Check that the old parameters are set for backward compatibility
    assert config.workspace_base == os.path.abspath('/repos/thinksoft/workspace')
    assert config.workspace_mount_path == os.path.abspath('/repos/thinksoft/workspace')
...
def test_defaults_dict_after_updates(default_config):
    ...
    updated_config = ThinksoftConfig()
...
[llm]
# Not set in `thinksoft/core/schema/config.py`
invalid_field = "test"
model = "gpt-4"
...
def test_agent_config_condenser_with_no_enabled():
    ...
    config = ThinksoftConfig(enable_default_condenser=False)
...
def test_api_keys_repr_str():
    ...
    # Test ThinksoftConfig
    app_config = ThinksoftConfig(
        llms={'llm': llm_config},
    ...
    # Check that no other attrs in ThinksoftConfig have 'key' or 'token' in their name
    # This will fail when new attrs are added, and attract attention
    known_key_token_attrs_app = [
        'search_api_key',
    ]
    for attr_name in ThinksoftConfig.model_fields.keys():
        if (
            not attr_name.startswith('__')
            and attr_name not in known_key_token_attrs_app
        ):
            assert 'key' not in attr_name.lower(), (
                f"Unexpected attribute '{attr_name}' contains 'key' in ThinksoftConfig"
            )
            assert 'token' not in attr_name.lower() or 'tokens' in attr_name.lower(), (
                f"Unexpected attribute '{attr_name}' contains 'token' in ThinksoftConfig"
            )
...
"""
    ...
    app_config = load_thinksoft_config(config_file=temp_toml_file)
