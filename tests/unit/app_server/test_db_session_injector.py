...
from thinksoft.app_server.services.db_session_injector import (  # noqa: E402
    DbSessionInjector,
)
...
class TestDbSessionInjectorConfiguration:
    ...
    def test_default_configuration(self, temp_persistence_dir):
        ...
        assert service.name == 'thinksoft'  # Default from env var processing
...
class TestDbSessionInjectorConnections:
    ...
    def test_sqlite_connection_fallback(self, basic_db_session_injector):
        ...
        expected_url = (
            f'sqlite:///{basic_db_session_injector.persistence_dir}/thinksoft.db'
        )
    ...
    async def test_sqlite_async_connection_fallback(self, basic_db_session_injector):
        ...
        expected_url = f'sqlite+aiosqlite:///{basic_db_session_injector.persistence_dir}/thinksoft.db'
        assert str(engine.url) == expected_url

    def test_postgres_connection_with_host(self, postgres_db_session_injector):
        """Test PostgreSQL connection when host is defined."""
        with patch(
            'thinksoft.app_server.services.db_session_injector.create_engine'
    ...
    async def test_postgres_async_connection_with_host(
        ...
        with patch(
            'thinksoft.app_server.services.db_session_injector.create_async_engine'
    ...
    @patch(
        'thinksoft.app_server.services.db_session_injector.DbSessionInjector._create_gcp_engine'
    )
    ...
    @patch(
        'thinksoft.app_server.services.db_session_injector.DbSessionInjector._create_async_gcp_engine'
    )
    @pytest.mark.asyncio
...
class TestDbSessionInjectorSessionManagement:
    ...
    async def test_depends_cleanup_on_completion(self, basic_db_session_injector):
        ...
        with patch(
            'thinksoft.app_server.services.db_session_injector.async_sessionmaker'
