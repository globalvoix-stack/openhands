...
def test_runtime_import_robustness():
    ...
    modules_to_clear = [k for k in sys.modules.keys() if 'thinksoft.runtime' in k]
    for module in modules_to_clear:
        del sys.modules[module]

    # Import the runtime module - should succeed even with broken third-party runtimes
    try:
        import thinksoft.runtime  # noqa: F401
...
def test_get_runtime_cls_works():
    ...
    import thinksoft.runtime

    # Test that we can still get core runtime classes
    docker_runtime = thinksoft.runtime.get_runtime_cls('docker')
    assert docker_runtime is not None

    local_runtime = thinksoft.runtime.get_runtime_cls('local')
    ...
    with pytest.raises(ValueError, match='Runtime nonexistent not supported'):
        thinksoft.runtime.get_runtime_cls('nonexistent')


def test_runtime_exception_handling():
    """Test that the runtime discovery code properly handles exceptions."""
    # This test verifies that the fix in thinksoft/runtime/__init__.py
    ...
    assert hasattr(thinksoft.runtime, 'get_runtime_cls')
    assert hasattr(thinksoft.runtime, '_THIRD_PARTY_RUNTIME_CLASSES')
...
def test_runtime_import_exception_handling_behavior():
    ...
    # Add our test handler to the Thinksoft logger
...
def test_import_error_handled_silently(caplog):
    ...
    logging.getLogger('thinksoft.runtime')
