...
from thinksoft.events.action import CmdRunAction, FileReadAction
from thinksoft.events.event import EventSource
from thinksoft.events.observation import ErrorObservation, FileReadObservation
...
def test_maybe_run_setup_script_executes_action():
    ...
    runtime.read.return_value = FileReadObservation(
        content="#!/bin/bash\necho 'test'", path='.thinksoft/setup.sh'
    )
    ...
    runtime.read.assert_called_once_with(FileReadAction(path='.thinksoft/setup.sh'))
    ...
    assert isinstance(action, CmdRunAction)
    assert (
        action.command == 'chmod +x .thinksoft/setup.sh && source .thinksoft/setup.sh'
    )
