...
from thinksoft.controller.state.control_flags import IterationControlFlag
...
from thinksoft.microagent import BaseMicroagent
...
def prompt_dir(tmp_path):
    # Copy contents from "thinksoft/agenthub/codeact_agent" to the temp directory
    shutil.copytree(
        'thinksoft/agenthub/codeact_agent/prompts', tmp_path, dirs_exist_ok=True
    )
...
def test_jinja2_template_inheritance(prompt_dir):
    ...
    with open(os.path.join(prompt_dir, 'system_prompt.j2'), 'w') as f:
        f.write("""You are Thinksoft agent, a helpful AI assistant that can interact with a computer to solve tasks.
...
""")
    ...
    assert 'You are Thinksoft agent' in base_msg
    ...
    assert 'You are Thinksoft agent' in interactive_msg
    ...
    assert 'You are Thinksoft agent' in long_horizon_msg
...
def test_prompt_manager_cli_mode_context(prompt_dir):
    ...
    with open(os.path.join(prompt_dir, 'system_prompt.j2'), 'w') as f:
        f.write("""You are Thinksoft agent.
...
Always be helpful and follow user instructions.
    ...
    assert 'You are Thinksoft agent' in cli_message
    ...
    assert 'You are Thinksoft agent' in sandbox_message
    ...
    assert 'You are Thinksoft agent' in default_message
