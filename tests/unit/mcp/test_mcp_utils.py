...
async def test_create_mcp_clients_empty():
    ...
    clients = await thinksoft.mcp.utils.create_mcp_clients([], [])
    assert clients == []


@pytest.mark.asyncio
@patch('thinksoft.mcp.utils.MCPClient')
async def test_create_mcp_clients_success(mock_mcp_client):
    ...
    clients = await thinksoft.mcp.utils.create_mcp_clients(server_configs, [])
...
def test_convert_mcp_clients_to_tools_empty():
    ...
    tools = thinksoft.mcp.utils.convert_mcp_clients_to_tools(None)
    assert tools == []

    tools = thinksoft.mcp.utils.convert_mcp_clients_to_tools([])
    assert tools == []
...
async def test_call_tool_mcp_no_clients():
    ...
    with pytest.raises(ValueError, match='No MCP clients found'):
        await thinksoft.mcp.utils.call_tool_mcp([], action)
...
async def test_call_tool_mcp_no_matching_client():
    ...
    with pytest.raises(ValueError, match='No matching MCP agent found for tool name'):
        await thinksoft.mcp.utils.call_tool_mcp([mock_client], action)
...
async def test_call_tool_mcp_success():
    ...
    # Call the function
    observation = await thinksoft.mcp.utils.call_tool_mcp([mock_client], action)
...
@pytest.mark.asyncio
@patch('thinksoft.mcp.utils.create_mcp_clients')
async def test_fetch_mcp_tools_from_config_with_stdio(mock_create_clients):
    ...
    tools = await thinksoft.mcp.utils.fetch_mcp_tools_from_config(
