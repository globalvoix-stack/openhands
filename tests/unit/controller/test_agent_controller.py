...
from thinksoft.events import Event, EventSource, EventStream, EventStreamSubscriber
from thinksoft.events.action import ChangeAgentStateAction, CmdRunAction, MessageAction
from thinksoft.events.action.agent import CondensationAction, RecallAction
...
from thinksoft.llm.llm_registry import LLMRegistry, RegistryEvent
from thinksoft.llm.metrics import Metrics, TokenUsage
from thinksoft.memory.condenser.condenser import Condensation
...
async def test_run_controller_with_fatal_error(
    ...
    with patch('thinksoft.core.main.create_agent', return_value=mock_agent):
...
async def test_context_window_exceeded_error_handling(
    ...
    config = ThinksoftConfig(max_iterations=max_iterations)
...
async def test_run_controller_with_context_window_exceeded_with_truncation(
    ...
    mock_runtime.event_stream = test_event_stream
    config = ThinksoftConfig(max_iterations=5)
    mock_runtime.config = copy.deepcopy(config)
...
async def test_run_controller_with_context_window_exceeded_without_truncation(
    ...
    mock_runtime.event_stream = test_event_stream
    config = ThinksoftConfig(max_iterations=3)
    mock_runtime.config = copy.deepcopy(config)
    try:
        # Mock the create_agent function to return our mock agent
        with patch('thinksoft.core.main.create_agent', return_value=mock_agent):
