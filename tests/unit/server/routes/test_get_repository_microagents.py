...
from thinksoft.server.dependencies import check_session_api_key
from thinksoft.server.routes.git import app as git_app
...
class TestGetRepositoryMicroagents:
    ...
    @pytest.mark.asyncio
    @patch('thinksoft.server.routes.git.ProviderHandler')
    async def test_get_microagents_github_success(
        ...
        mock_provider_handler.get_microagents.return_value = [
            {
                'name': 'test_agent',
                'path': '.thinksoft/microagents/test_agent.md',
...
class TestGetRepositoryMicroagentContent:
    ...
    async def test_get_microagent_content_github_success(
        ...
        # Execute test
        file_path = '.thinksoft/microagents/test_agent.md'
    ...
    async def test_get_microagent_content_file_not_found(
        ...
        # Execute test
        file_path = '.thinksoft/microagents/nonexistent.md'
        response = test_client.get(
            f'/api/user/repository/test/repo/microagents/content?file_path={quote(file_path)}'
        )
...
class TestSpecialRepositoryStructures:
    ...
    @pytest.mark.asyncio
    @patch('thinksoft.server.routes.git.ProviderHandler')
    async def test_get_microagents_thinksoft_repo_structure(
        self,
        mock_provider_handler_cls,
        test_client,
    ):
        """Test microagents from .thinksoft repository structure."""
        # Setup mocks
        mock_provider_handler = AsyncMock()
        mock_provider_handler_cls.return_value = mock_provider_handler

        # Mock the get_microagents method to return sample data for .thinksoft repo
        mock_provider_handler.get_microagents.return_value = [
            {
                'name': 'test_agent',
                'path': 'microagents/test_agent.md',  # Should be in microagents folder, not .thinksoft/microagents
                'created_at': '2024-01-01T00:00:00',
            }
        ]

        # Execute test
        response = test_client.get('/api/user/repository/test/.thinksoft/microagents')
        ...
        )  # Should be in microagents folder, not .thinksoft/microagents

    @pytest.mark.asyncio
    @patch('thinksoft.server.routes.git.ProviderHandler')
    async def test_get_microagents_gitlab_thinksoft_config_structure(
        self,
        mock_provider_handler_cls,
        test_client,
    ):
        """Test microagents from GitLab thinksoft-config repository structure."""
        # Setup mocks
        mock_provider_handler = AsyncMock()
        mock_provider_handler_cls.return_value = mock_provider_handler

        # Mock the get_microagents method to return sample data for thinksoft-config repo
        mock_provider_handler.get_microagents.return_value = [
            {
                'name': 'test_agent',
                'path': 'microagents/test_agent.md',  # Should be in microagents folder, not .thinksoft/microagents
        ...
        # Execute test
        response = test_client.get(
            '/api/user/repository/test/thinksoft-config/microagents'
        )
