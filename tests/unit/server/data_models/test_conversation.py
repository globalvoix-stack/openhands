...
from thinksoft.app_server.app_conversation.app_conversation_router import (
...
from thinksoft.sdk.workspace.models import FileOperationResult
from thinksoft.sdk.workspace.remote.async_remote_workspace import (
...
from thinksoft.server.routes.manage_conversations import app as conversation_app
...
def _patch_store():
    ...
    with patch(
        'thinksoft.storage.conversation.file_conversation_store.get_file_store',
        MagicMock(return_value=file_store),
    ):
        with patch(
            'thinksoft.server.routes.manage_conversations.conversation_manager.file_store',
            file_store,
        ):
            yield
...
def provider_handler_mock():
    with patch(
        'thinksoft.server.routes.manage_conversations.ProviderHandler'
    ) as mock_cls:
...
async def test_search_conversations():
    with _patch_store():
        with patch(
            'thinksoft.server.routes.manage_conversations.config'
        ) as mock_config:
...
async def test_new_conversation_success(provider_handler_mock):
    ...
    with _patch_store():
        ...
        with patch(
            'thinksoft.server.routes.manage_conversations.create_new_conversation'
...
async def test_new_conversation_with_suggested_task(provider_handler_mock):
    ...
    with _patch_store():
        ...
        with patch(
            ...
            with patch(
                'thinksoft.integrations.service_types.SuggestedTask.get_prompt_for_task'
            ) as mock_get_prompt:
...
async def test_delete_conversation():
    with _patch_store():
        ...
        with patch(
            'thinksoft.server.routes.manage_conversations.ConversationStoreImpl.get_instance'
            ...
            with patch(
                ...
                with patch(
                    'thinksoft.server.routes.manage_conversations.get_runtime_cls'
...
async def test_delete_v1_conversation_success():
    ...
    with patch(
        'thinksoft.server.routes.manage_conversations.app_conversation_service_dependency'
    ) as mock_service_dep:
        ...
        with patch(
            'thinksoft.server.routes.manage_conversations.app_conversation_info_service_dependency'
            ...
            with patch(
                'thinksoft.server.routes.manage_conversations.sandbox_service_dependency'
...
async def test_delete_v1_conversation_with_sub_conversations():
    ...
    service = LiveStatusAppConversationService(
        ...
        httpx_client=mock_httpx_client,
        web_url=None,
        thinksoft_provider_base_url=None,
...
async def test_read_conversation_file_success():
    ...
    with patch(
        'thinksoft.app_server.app_conversation.app_conversation_router.AsyncRemoteWorkspace'
        ...
        with patch(
            'thinksoft.app_server.app_conversation.app_conversation_router.tempfile.NamedTemporaryFile'
        ) as mock_tempfile:
            ...
            with patch('builtins.open', create=True) as mock_open:
                ...
                with patch(
                    'thinksoft.app_server.app_conversation.app_conversation_router.os.unlink'
                ) as mock_unlink:
                    # Call the endpoint
