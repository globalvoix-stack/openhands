...
async def test_setup_with_provided_tokens_uses_real_tokens(
    ...
    with patch(
        'thinksoft.server.services.conversation_service.SettingsStoreImpl.get_instance'
    ) as mock_settings_store_cls:
        with patch(
            'thinksoft.server.services.conversation_service.SecretsStoreImpl.get_instance'
        ) as mock_secrets_store_cls:
            with patch(
                'thinksoft.server.services.conversation_service.server_config'
...
async def test_setup_without_tokens_non_saas_uses_user_secrets(mock_settings):
    """Test that Thinksoft (OSS mode) uses user_secrets.provider_tokens when no tokens provided.

    This test verifies Thinksoft (OSS mode) backward compatibility - tokens come from local config, not endpoint.
    """
    user_id = 'test_user_456'
    conversation_id = 'test_conv_123'
    providers_set = [ProviderType.GITHUB]
    ...
    with patch(
        ...
        with patch(
            ...
            with patch(
                ...
                mock_server_config.app_mode = AppMode.THINKSOFT
