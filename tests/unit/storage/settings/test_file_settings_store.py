...
async def test_load_nonexistent_data(file_settings_store):
    with patch(
        'thinksoft.storage.data_models.settings.load_thinksoft_config',
        MagicMock(return_value=ThinksoftConfig()),
    ):
...
async def test_get_instance():
    config = ThinksoftConfig(file_store='local', file_store_path='/test/path')

    with patch(
        'thinksoft.storage.settings.file_settings_store.get_file_store'
    ) as mock_get_store:
