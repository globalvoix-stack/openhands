# BioCoder Evaluation with Openthinksoft
...
In the thinksoft branch of the Biocoder repository, we have slightly modified our original Docker image to work with the Thinksoft environment. In the Docker image are testing scripts (`/testing/start_test_thinksoft.py` and aux files in `/testing_files/`) to assist with evaluation. Additionally, we have installed all dependencies, including OpenJDK, mamba (with Python 3.6), and many system libraries. Notably, we have **not** packaged all repositories into the image, so they are downloaded at runtime.
...
To reproduce this image, please see the Dockerfile_Openthinksoft in the `biocoder` repository.
...
with current Thinksoft version, then your command would be:
