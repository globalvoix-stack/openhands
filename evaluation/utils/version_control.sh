checkout_eval_branch() {
    ...
    echo "Start to checkout thinksoft version to $COMMIT_HASH, but keep current evaluation harness"
    ...
    echo "Check out Thinksoft to version: $COMMIT_HASH"
...
get_thinksoft_version() {
    # IMPORTANT: Because Agent's prompt changes fairly often in the rapidly evolving codebase of Thinksoft
    # We need to track the version of Agent in the evaluation to make sure results are comparable
    THINKSOFT_VERSION=v$(poetry run python -c "from thinksoft import get_version; print(get_version())")
}
